...
while any(v != 0 for v in y):
    break  ##### break to test extra credit 
    numberofloops += 1
    print('\n')
...
alldata['n']=len(M)
alldata['lambda']=3.0
alldata['lower']= [np.amin(returns[i]) for i in range(n)]
alldata['upper']=[np.amax(returns[i]) for i in range(n)]
